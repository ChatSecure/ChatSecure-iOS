...
static NSTimeInterval const kOTRMessagesGroupViewController = 5 * 60;
...
@interface OTRMessagesGroupViewController () <OTRMessagesCollectionViewCellDelegate>
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    __weak OTRMessagesGroupViewController *welf = self;
    ...
    [self.uiDatabaseConnection readWithBlock:^(YapDatabaseReadTransaction *transaction) {
        ...
        [welf.groupMappings updateWithTransaction:transaction];
    }];
...
- (void)setBroadcastGroup:(OTRBroadcastGroup *)broadcastGroup
{
    ...
    if ([originalGroup.uniqueId isEqualToString:broadcastGroup.uniqueId]) {
        ...
        if (self.broadcastGroup) {
            ...
            self.messageMappings = [[YapDatabaseViewMappings alloc] initWithGroups:@[self.broadcastGroup.uniqueId] view:OTRBroadcastChatDatabaseViewExtensionName];
            ...
            [self.uiDatabaseConnection readWithBlock:^(YapDatabaseReadTransaction *transaction) {
                self.account = [self.broadcastGroup accountWithTransaction:transaction];
...
{
    if ([self.broadcastGroup.displayName length]) {
        self.titleView.titleLabel.text = self.broadcastGroup.displayName;
    }
...
- (void)setupLockButton
{
    ...
    self.lockButton = [OTRLockButton lockButtonWithInitailLockStatus:OTRLockStatusUnlocked withBlock:^(OTRLockStatus currentStatus) {
        ...
        void (^showEncryptionDropDown)(void) = ^void(void) {
            ...
            for (OTRBuddy *buddy in self.broadcastGroup.buddies)
            {
                [[OTRKit sharedInstance] messageStateForUsername:buddy.username accountName:welf.account.username protocol:welf.account.protocolTypeString completion:^(OTRKitMessageState messageState) {
    ...
    [[OTRKit sharedInstance] checkIfGeneratingKeyForAccountName:self.account.username protocol:self.account.protocolTypeString completion:^(BOOL isGeneratingKey) {
        if( isGeneratingKey) {
            ...
            if ([rightBarItem isEqual:self.lockBarButtonItem]) {
                ...
                for (OTRBuddy *buddy in self.broadcastGroup.buddies)
                {
                    [[OTRKit sharedInstance] activeFingerprintIsVerifiedForUsername:buddy.username accountName:self.account.username protocol:self.account.protocolTypeString completion:^(BOOL isTrusted) {
                        
                        [[OTRKit sharedInstance] hasVerifiedFingerprintsForUsername:buddy.username accountName:self.account.username protocol:self.account.protocolTypeString completion:^(BOOL hasVerifiedFingerprints) {
    ...
    [[OTRKit sharedInstance] fingerprintForAccountName:self.account.username protocol:self.account.protocolTypeString completion:^(NSString *ourFingerprintString) {
        ...
        [[OTRKit sharedInstance] activeFingerprintForUsername:self.buddy.username accountName:self.account.username protocol:self.account.protocolTypeString completion:^(NSString *theirFingerprintString) {
            ...
            [[OTRKit sharedInstance] activeFingerprintIsVerifiedForUsername:self.buddy.username accountName:self.account.username protocol:self.account.protocolTypeString completion:^(BOOL verified) {
                ...
                UIAlertView * alert;
                __weak OTRGroupDifussionMessagesViewController * welf = self;
...
- (void)saveCurrentMessageText
{
    ...
    self.broadcastGroup.composingMessageString = self.inputToolbar.contentView.textView.text;
    /*if(![self.broadcastGroup.composingMessageString length])
    {
        [self.xmppManager sendChatState:kOTRChatStateInactive withBuddyID:self.broadcastGroup.uniqueId];
    }*/
}
...
- (OTRMessage *)messageAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self.uiDatabaseConnection readWithBlock:^(YapDatabaseReadTransaction *transaction) {
        YapDatabaseViewTransaction *viewTransaction = [transaction ext:OTRBroadcastChatDatabaseViewExtensionName];
...
- (BOOL)showDateAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        if (timeDifference > kOTRMessagesGroupViewController) {
            showDate = YES;
        }
    }
    return showDate;
...
- (void)didPressSendButton:(UIButton *)button withMessageText:(NSString *)text sender:(NSString *)sender date:(NSDate *)date
{
    ...
    message.broadcastGroupUniqueId = self.broadcastGroup.uniqueId;
    message.text = text;
    message.read = YES;
    ...
    for (OTRBuddy *buddy in self.broadcastGroup.buddies)
    {
        ...
        message2.buddyUniqueId = buddy.uniqueId;
        message2.text = text;
        message2.read = YES;
        message2.transportedSecurely = NO;
        message2.isBroadcastMessage = YES;
    
        [[OTRDatabaseManager sharedInstance].readWriteDatabaseConnection asyncReadWriteWithBlock:^(YapDatabaseReadWriteTransaction *transaction) {
            [message2 saveWithTransaction:transaction];
        } completionBlock:^{   
            [[OTRKit sharedInstance] encodeMessage:message2.text tlvs:nil username:buddy.username accountName:self.account.username protocol:self.account.protocolTypeString tag:message2];
        }];
    }
}
...
- (void)yapDatabaseModified:(NSNotification *)notification
{
    ...
    [[self.uiDatabaseConnection ext:OTRBroadcastChatDatabaseViewExtensionName] getSectionChanges:nil
