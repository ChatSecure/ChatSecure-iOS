...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
	...
	[self setupStream];
    chatManager = [[ChatWindowManager alloc] init];
    
    rootController = [[[RootViewController alloc] init] retain];
...
- (void)xmppStream:(XMPPStream *)sender didReceiveMessage:(XMPPMessage *)message
{
	...
	{
        [chatManager handleChatMessage:message withStream:sender];
...
-(void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    if(alertView.tag == 444)
    {
        if(buttonIndex == 1)
        {            
            XMPPUserCoreDataStorageObject *user = [xmppRosterStorage userForJID:[chatManager.backgroundMessage from]
                                                                     xmppStream:chatManager.backgroundStream
                                                           managedObjectContext:[self managedObjectContext_roster]];
            
            [chatManager openChatWindowWithStream:chatManager.backgroundStream forUser:user];
